...
with open('all_seam_cells.csv', 'w') as f: 
    count_id = 0
    writer = csv.writer(f, delimiter=' ', quotechar='|') # need to make sure that there are spaces between values
    ...
    for file in csv_filenames:
        ...
        with open(file, 'r') as g:
            ...
            for row in reader:
                ...
                id = count_id
                count_id += 1
                writer.writerow([frame, z, y, x]) # name, id - remove these values since they cannot be converted to float
