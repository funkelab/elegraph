...
def calc_f1_volumes(predicted_volume, gt_volume, radius):
    ...
    gt_local_max = find_local_max(gt_volume, 1) # ground truth maxes are where the coordinate's intensity is 1 due to gaussian blobs
...
def test_f1(voxel_size, radius, pred_volume, gt_volume):
    ...
    for time in range(min_time, max_time + 1):
        ...
        for row in time_matrix:
            ...
            total_f1 += calc_f1_volumes(cut_pred, cut_gt) # won't the cut_pred sometimes include blobs we've already seen in training/validation?
            count += 1
